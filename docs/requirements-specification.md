...
- 系统不保证官方实时比分，数据源为本地课程演示库；
- 系统不保存用户账号和长期对话记录；
- 前端展示的首页样例用于产品形态演示，真实智能体查询入口为 `/api/chat`；
- `result_payload` 当前为可选扩展字段，基础验收以 `answer`、`tool_calls`、`error` 为准。
...
| `result_payload` | object / null | 否 | 可选结构化展示数据，当前基础版本可为 null |
...
| `history` | 前端维护的对话历史 |
| `result_payload` | 可选结构化展示数据，当前基础版本主要使用 `answer` |
